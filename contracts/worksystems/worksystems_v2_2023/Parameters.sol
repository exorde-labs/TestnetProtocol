...
contract Parameters is Ownable {
    ...
    uint256 public QUALITY_DATA_BATCH_SIZE = 1;
    uint256 public QUALITY_MIN_CONSENSUS_WORKER_COUNT = 2;
    uint256 public QUALITY_MAX_CONSENSUS_WORKER_COUNT = 4;
    uint256 public QUALITY_MIN_STAKE = 25 * (10**18);
    uint256 public QUALITY_COMMIT_ROUND_DURATION = 400;
    uint256 public QUALITY_REVEAL_ROUND_DURATION = 180;
    uint256 public QUALITY_MIN_REWARD_DataValidation = 1 * (10**15);
    uint256 public QUALITY_MIN_REP_DataValidation = 10 * (10**15);
    ...
    function updateContractsAddresses(
        ...
        if (QualitySystem_ != address(0)) {
            QualitySystem = QualitySystem_;
        }
    ...
    function updateQualityParameters(uint256 ParameterIndex, uint256 uintValue) public onlyOwner {
        if (ParameterIndex == 1) {
            QUALITY_DATA_BATCH_SIZE = uintValue;
        }
        if (ParameterIndex == 2) {
            QUALITY_MIN_STAKE = uintValue;
        }
        if (ParameterIndex == 3) {
            QUALITY_MIN_CONSENSUS_WORKER_COUNT = uintValue;
        }
        if (ParameterIndex == 4) {
            QUALITY_MAX_CONSENSUS_WORKER_COUNT = uintValue;
        }
        if (ParameterIndex == 5) {
            QUALITY_COMMIT_ROUND_DURATION = uintValue;
        }
        if (ParameterIndex == 6) {
            QUALITY_REVEAL_ROUND_DURATION = uintValue;
        }
        if (ParameterIndex == 7) {
            QUALITY_MIN_REP_DataValidation = uintValue;
        }
        if (ParameterIndex == 8) {
            QUALITY_MIN_REWARD_DataValidation = uintValue;
        }
    }
    ...
    function getQualitySystem() public view returns (address) {
    ...
    // -------------- GETTERS : Quality --------------------
    function get_QUALITY_DATA_BATCH_SIZE() public view returns (uint256) {
        return QUALITY_DATA_BATCH_SIZE;
    }

    function get_QUALITY_MIN_STAKE() public view returns (uint256) {
        return QUALITY_MIN_STAKE;
    }

    function get_QUALITY_MIN_CONSENSUS_WORKER_COUNT() public view returns (uint256) {
        return QUALITY_MIN_CONSENSUS_WORKER_COUNT;
    }

    function get_QUALITY_MAX_CONSENSUS_WORKER_COUNT() public view returns (uint256) {
        return QUALITY_MAX_CONSENSUS_WORKER_COUNT;
    }

    function get_QUALITY_COMMIT_ROUND_DURATION() public view returns (uint256) {
        return QUALITY_COMMIT_ROUND_DURATION;
    }

    function get_QUALITY_REVEAL_ROUND_DURATION() public view returns (uint256) {
        return QUALITY_REVEAL_ROUND_DURATION;
    }

    function get_QUALITY_MIN_REWARD_DataValidation() public view returns (uint256) {
        return QUALITY_MIN_REWARD_DataValidation;
    }

    function get_QUALITY_MIN_REP_DataValidation() public view returns (uint256) {
        return QUALITY_MIN_REP_DataValidation;
    }
