...
contract RandomSubsets is RandomAllocator {
    ...
    */
    function _randomSubsets(uint128 K, uint128 N, uint128 L, uint128 M, uint128 maxFailedAttempts) private view returns (uint256[][] memory) {
        ...
        uint256[] memory generatedIndices = new uint256[](K);
    ...
    */
    function getStandardSubsets(uint128 K_, uint128 N_, uint128 coverage) public view returns (uint256[][] memory) {
    ...
    /**
        ...
        We approximate this to 15 in general.
    ...
    */
    function _getParameters(uint128 K_, uint128 N_, uint128 coverage) private pure returns (uint128, uint128, uint128) {
        ...
        uint128 maxFailedAttempts_ = 15; // Maximum failed attempts
        // Generate the random non-overlapping subsets using the calculated minimum and maximum subset lengths and maximum failed attempts.
        if ( M_ == 0 || L_ == 0 ){
            M_ = N_ / 10 + 1;
            L_ = M_ * 4;
    ...
    */
    function getCoverage(uint128 K_, uint128 N_, uint128 coverage) public view returns (uint256, uint256[] memory, uint256, uint256) {
        uint256[][] memory subsets = getStandardSubsets(K_, N_, coverage);
        uint256[] memory subset_lengths = new uint256[](subsets.length);
        uint256 cumulative_length;
        uint256 nb_subsets = 0;
        uint256 coverage_percentage = 0;
        for (uint i = 0; i < subsets.length; i++) {
            if (subsets[i].length > 0) { // Check if subset is not empty
                uint256 length = subsets[i][1] - subsets[i][0] + 1;
