...
contract DataIndexing is Ownable, RandomAllocator {
    ...
    */
                ...
                if (AreStringsEqual(worker_proposed_hash, majorityNewFile)) {
                    ...
                    require(
                        _RepManager.mintReputationForWork(
                            Parameters.get_INDEXING_MIN_REP_DataValidation() * majorityBatchCount,
                            worker_master_addr_,
                            ""
                        ),
                    ...
                    require(
                        _RewardsManager.ProxyAddReward(
                            Parameters.get_INDEXING_MIN_REWARD_DataValidation() * majorityBatchCount,
                            worker_master_addr_
                        ),
    ...
    function AllocateWork() public {
        ...
        uint256 selected_k = Math.max(
            Math.min(availableWorkers.length, Parameters.get_INDEXING_MAX_CONSENSUS_WORKER_COUNT()),
            Parameters.get_INDEXING_MIN_CONSENSUS_WORKER_COUNT()
    ...
    */
    function revealIndexingCheck(
        ...
        if (_voteOption == 1) {
            // apply numTokens to appropriate IndexingData choice
    ...
    */
    function getNumPassingTokens(
        ...
        uint256 _DataBatchId,
        uint256 _salt
    ) public view returns (uint256 correctIndexingChecks) {
