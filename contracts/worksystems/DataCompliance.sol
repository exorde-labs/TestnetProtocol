...
library DLL2 {
    ...
    function isEmpty(ComplianceData storage self) public view returns (bool) {
        return getStart(self) == NULL_NODE_ID;
    }

    function contains(ComplianceData storage self, uint256 _curr) public view returns (bool) {
    ...
    function getNext(ComplianceData storage self, uint256 _curr) public view returns (uint256) {
        return self.dll[_curr].next;
    }

    function getPrev(ComplianceData storage self, uint256 _curr) public view returns (uint256) {
        return self.dll[_curr].prev;
    }

    function getStart(ComplianceData storage self) public view returns (uint256) {
        return getNext(self, NULL_NODE_ID);
    }

    function getEnd(ComplianceData storage self) public view returns (uint256) {
  ...
  */
    ...
    function remove(ComplianceData storage self, uint256 _curr) public {
...
contract DataCompliance is Ownable, RandomAllocator {
    ...
    // ============ EVENTS ============
    event _ComplianceSubmitted(uint256 indexed DataID, string file_hash, address sender);
    event _ComplianceCheckCommitted(uint256 indexed DataID, uint256 numTokens, address indexed voter);
    event _ComplianceCheckRevealed(
    ...
    event _ComplianceAccepted(string hash, address indexed creator);
    ...
    // ============ LIBRARIES ============
    using AttributeStore2 for AttributeStore2.ComplianceData;
    using DLL2 for DLL2.ComplianceData;
    ...
    struct BatchMetadata {
        ...
        uint256 commitEndDate; // expiration date of commit period for poll
        uint256 revealEndDate; // expiration date of reveal period for poll
        uint256 votesFor; // tally of Compliance-check-votes supporting proposal
        uint256 votesAgainst; // tally of Compliance-check-votes countering proposal
        string batchIPFSfile; // to be updated during ComplianceChecking
        DataStatus status; // state of the vote
    }

    // ------ Atomic Data Structure
    struct ComplianceData {
        string ipfs_hash; // expiration date of commit period for ComplianceData
        address author; // author of the proposal
        uint256 timestamp; // expiration date of commit period for ComplianceData
        DataStatus status; // state of the vote
    }
    ...
    mapping(address => mapping(uint256 => bool)) public UserChecksCommits; // indicates whether an address committed a Compliance-check-vote for this poll
    mapping(address => mapping(uint256 => bool)) public UserChecksReveals; // indicates whether an address revealed a Compliance-check-vote for this poll
    ...
    // ------ Backend Data Stores
    mapping(address => DLL2.ComplianceData) dllMap;
    AttributeStore2.ComplianceData store;
    mapping(uint256 => ComplianceData) public CompliancesMapping; // maps DataID to ComplianceData struct
    mapping(uint256 => BatchMetadata) public DataBatch; // refers to ComplianceData indices
    ...
    */
    ...
    function deleteData(uint256 _DataId) public onlyOwner {
        delete CompliancesMapping[_DataId];
    }
    ...
    function Ping(uint256 CheckedBatchId) public {
        ...
        if (SpottingSystem != ISpottingSystem(address(0)) && !CollectedSpotBatchs[CheckedBatchId]) {
            ...
            if (SpottingSystem.DataExists(CheckedBatchId)) {
                ...
                if (SpotBatchStatus == ISpottingSystem.DataStatus.APPROVED) {
                    // -------- ADDING NEW CHECKED SPOT BATCH AS A NEW ITEM IN OUR COMPLIANCE BATCH --------

                    CompliancesMapping[DataNonce] = ComplianceData({
                    ...
                    if (current_data_batch.counter < Parameters.get_COMPLIANCE_DATA_BATCH_SIZE()) {
                    ...
                    emit _ComplianceSubmitted(DataNonce, SpotBatch.batchIPFSfile, msg.sender);
                }
                // }
    ...
    function TriggerUpdate() public topUpSFuel {
        ...
        for (uint256 i = 0; i < Parameters.get_MAX_UPDATE_ITERATIONS(); i++) {
            ...
            if (
                ...
                availableWorkers.length >= Parameters.get_COMPLIANCE_MIN_CONSENSUS_WORKER_COUNT() &&
    ...
    /**
    @notice Trigger the validation of a ComplianceData hash; if the ComplianceData has ended. If the requirements are APPROVED, 
    ...
    */
    function ValidateDataBatch(uint256 _DataBatchId) public {
                ...
                if (AreStringsEqual(worker_proposed_hash, majorityNewFile)) {
                    ...
                    require(
                        _RepManager.mintReputationForWork(
                            Parameters.get_COMPLIANCE_MIN_REP_DataValidation() * majorityBatchCount,
                            worker_master_addr_,
                            ""
                        ),
                    ...
                    require(
                        _RewardsManager.ProxyAddReward(
                            Parameters.get_COMPLIANCE_MIN_REWARD_DataValidation() * majorityBatchCount,
                            worker_master_addr_
                        ),
        ...
        bool isCheckPassed = isPassed(_DataBatchId) &&
            (AreStringsEqual(majorityNewFile, "") == false) &&
        ...
        if (isCheckPassed) {
            ...
            try _ArchivingSystem.Ping(_DataBatchId) returns (bool) {} catch (bytes memory err) {
                emit BytesFailure(err);
            }
            try _IndexingSystem.Ping(_DataBatchId) returns (bool) {} catch (bytes memory err) {
                emit BytesFailure(err);
            }
        }
        // -------------------------------------------------------------
        ...
        emit _ComplianceAccepted(CompliancesMapping[_DataBatchId].ipfs_hash, CompliancesMapping[_DataBatchId].author);
    }
    ...
    function AllocateWork() public {
        ...
        uint256 selected_k = Math.max(
            Math.min(availableWorkers.length, Parameters.get_COMPLIANCE_MAX_CONSENSUS_WORKER_COUNT()),
            Parameters.get_COMPLIANCE_MIN_CONSENSUS_WORKER_COUNT()
        ...
        uint256 _commitEndDate = block.timestamp.add(Parameters.get_COMPLIANCE_COMMIT_ROUND_DURATION());
        uint256 _revealEndDate = _commitEndDate.add(Parameters.get_COMPLIANCE_REVEAL_ROUND_DURATION());
    ...
    /**
    @notice Commits Compliance-check-vote using hash of choice and secret salt to conceal Compliance-check-vote until reveal
    @param _DataBatchId Integer identifier associated with target ComplianceData
    ...
    */
    ...
    function commitComplianceCheck(
        ...
        AllTxsCounter += 1;
        emit _ComplianceCheckCommitted(_DataBatchId, _numTokens, msg.sender);
    }

    /**
    @notice Reveals Compliance-check-vote with choice and secret salt used in generating commitHash to attribute committed tokens
    @param _DataBatchId Integer identifier associated with target ComplianceData
    ...
    */
    function revealComplianceCheck(
        uint256 _DataBatchId,
        uint256 _voteOption,
        ...
        if (_voteOption == 1) {
            // apply numTokens to appropriate ComplianceData choice
        ...
        // ----------------------- USER STATE UPDATE -----------------------
        dllMap[msg.sender].remove(_DataBatchId); // remove the node referring to this Compliance-check-vote upon reveal
    ...
    */
    ...
    /**
    @dev Unlocks tokens locked in unrevealed Compliance-check-vote where ComplianceData has ended
    @param _DataBatchId Integer identifier associated with the target ComplianceData
    */
    ...
    /**
    @dev Unlocks tokens locked in unrevealed Compliance-check-votes where Datas have ended
    ...
    */
    ...
    function getIPFShashesForBatch(uint256 _DataBatchId) public view returns (string[] memory) {
        ...
        for (uint256 i = 0; i < batch_size; i++) {
            ...
            string memory ipfs_hash_ = CompliancesMapping[k].ipfs_hash;
    ...
    /**
    @dev Compares previous and next ComplianceData's committed tokens for sorting purposes
    ...
    */
    function getNumPassingTokens(
        ...
        uint256 _DataBatchId,
        uint256 _salt
    ) public view returns (uint256 correctComplianceChecks) {
    ...
    */
    function getTotalNumberOfComplianceChecks(uint256 _DataBatchId) public view returns (uint256 vc) {
        // Build ComplianceCheckings Struct
    ...
    */
    ...
    /**
    @notice Determines if ComplianceData is over
    ...
    */
    ...
    /**
    @notice Checks if the reveal period is still active for the specified ComplianceData
    ...
    */
    ...
    /**
    @dev Checks if user has committed for specified ComplianceData
    ...
    */
    function didCommit(address _voter, uint256 _DataBatchId) public view returns (bool committed) {
        ...
        // return CompliancesMapping[_DataBatchId].didCommit[_voter];
        return UserChecksCommits[_voter][_DataBatchId];
    }

    /**
    @dev Checks if user has revealed for specified ComplianceData
    @param _voter Address of user to check against
    @param _DataBatchId Integer identifier associated with target ComplianceData
    ...
    */
    function didReveal(address _voter, uint256 _DataBatchId) public view returns (bool revealed) {
        ...
        // return CompliancesMapping[_DataBatchId].didReveal[_voter];
        return UserChecksReveals[_voter][_DataBatchId];
    }

    /**
    @dev Checks if a ComplianceData exists
    @param _DataBatchId The DataID whose existance is to be evaluated.
    @return exists Boolean Indicates whether a ComplianceData exists for the provided DataID
    */
    ...
    */
    function getDataByID(uint256 _DataId) public view returns (ComplianceData memory data) {
        return CompliancesMapping[_DataId];
    }
    ...
    */
    ...
    /**
    @dev Gets the bytes32 commitHash property of target ComplianceData
    @param _hash ipfs hash of aggregated data in a string
    @param _salt is the salt
    @return keccak256hash Bytes32 hash property attached to target ComplianceData
    */
    ...
    @param _voter Address of user to check against
    @param _DataBatchId Integer identifier associated with target ComplianceData
    @return numTokens Number of tokens committed to ComplianceData in sorted ComplianceData-linked-list
    */
    ...
    /**
    @dev Gets top element of sorted ComplianceData-linked-list
    @param _voter Address of user to check against
    @return DataID Integer identifier to ComplianceData with maximum number of tokens committed to it
    */
    ...
    @param _voter Address of user to check against
    @return numTokens Maximum number of tokens committed in ComplianceData specified
    */
    ...
    */
    ...
    /**
    @dev Generates an identifier which associates a user and a ComplianceData together
    @param _DataBatchId Integer identifier associated with target ComplianceData
