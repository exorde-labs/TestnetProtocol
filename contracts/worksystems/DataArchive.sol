...
contract DataArchive is Ownable, RandomAllocator {
    ...
    */
    ...
    function updateStakeManager(address addr) public onlyOwner {
        StakeManager = IStakeManager(addr);
    }

    function updateRepManager(address addr) public onlyOwner {
        RepManager = IRepManager(addr);
    }

    function updateRewardManager(address addr) public onlyOwner {
        RewardManager = IRewardManager(addr);
    }

    function updatePreviousSystem(address addr) public onlyOwner {
        FormattingSystem = IFormattingSystem(addr);
    }

    function updateAddressManager(address addr) public onlyOwner {
    ...
    function _retrieveSFuel() internal {
        ...
        ) = sFuel.call(abi.encodeWithSignature("retrieveSFuel(address)", payable(msg.sender)));
        (
            bool success2, /* bytes memory data2 */

        ) = sFuel.call(abi.encodeWithSignature("retrieveSFuel(address payable)", payable(msg.sender)));
    ...
    function Ping(uint256 CheckedBatchId) public returns (bool) {
        if (FormattingSystem != IFormattingSystem(address(0)) && !CollectedFormatBatchs[CheckedBatchId]) {
    ...
    function deleteMapping(uint256 _CheckedBatchId) public onlyOwner {
