...
contract Controller is ControllerInterface {
    struct Scheme {
        ...
        // 7th bit: Scheme can use StakingManager ? (not used yet)
    }
    ...
    modifier onlyStakingScheme() {
        require(schemes[msg.sender].permissions & bytes4(0x00000040) == bytes4(0x00000040));
        _;
    }
     ...
     * @return success which represents a success
     */
    function addGlobalConstraint(
        address _globalConstraint,
        bytes32 _params,
        address _avatar
    ) external onlyGlobalConstraintsScheme isAvatarValid(_avatar) returns (bool success) {
    ...
    // ------------------------------------------------------------------------
    // ------------------------------------------------------------------------
    // STAKING AND REWARDS MANAGER
    // ------------------------------------------------------------------------
    // ------------------------------------------------------------------------

    /**
     * @dev upgrade the Controller
     *      The function will trigger an event '_stakingManager'.
     * @param  _stakingManager the address of the new controller.
     * @return bool which represents a success
     */
    function upgradeStakingManager(address _stakingManager) external onlyUpgradingScheme returns (bool) {
     ...
     * @return success which represents a success
     */
    function upgradeRewardsManager(address _newController, Avatar _avatar)
        external
        onlyUpgradingScheme
        isAvatarValid(address(_avatar))
        returns (bool success)
     ...
     * @param _to address of the beneficiary
     * @param _value the amount of ether (in Wei) to send
     * @return success boolean which represents a success
     */
    ...
    function isStakingManagerRegistered(address _stakingManagerAddress, address _avatar)
        external
        view
        isAvatarValid(_avatar)
        returns (bool success)
    {
        return _isStakingManager(_stakingManagerAddress);
    }
    ...
    function _isStakingManager(address _stakingManager) private view returns (bool success) {
        return (_stakingManager == address(stakingManagerAddress));
    }
}
