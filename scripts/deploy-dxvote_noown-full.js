...
task("deploy-dxvote", "Deploy dxvote in localhost network")
  ...
  .setAction(async ({ deployconfig }) => {
    ...
    await avatar.transferOwnership(owner);
    await reputation.transferOwnership(owner);
    ...
    console.log("Give back all ownerships to the owner... (StakingManager & Worksystems)\n");
    await spot_worksystem.transferOwnership(owner);
    await staking_manager.transferOwnership(owner);
